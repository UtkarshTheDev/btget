...
export class PieceAssembler {
  ...
  constructor(torrent: Torrent) {
    const pieceHashes = this.extractPieceHashes(torrent.info.pieces);
    this.totalPieces = pieceHashes.length;
    this.pieceLength = torrent.info['piece length'] as number;
    this.totalLength = this.calculateTotalLength(torrent);
    ...
    console.log(`🧩 Initialized ${this.totalPieces} pieces (${this.pieceLength} bytes each, last: ${this.calculatePieceSize(this.totalPieces - 1)} bytes)`);
  }
  ...
  private extractPieceHashes(piecesBuffer: Buffer): Buffer[] {
    ...
    for (let i = 0; i < piecesBuffer.length; i += 20) {
      hashes.push(piecesBuffer.slice(i, i + 20));
    }
    return hashes;
  }
  ...
  private calculateTotalLength(torrent: Torrent): bigint {
    if (torrent.info.files) {
      return torrent.info.files.reduce((sum, file) => sum + BigInt(file.length), BigInt(0));
    } else {
  ...
  private verifyAndCompletePiece(pieceIndex: number): boolean {
    ...
    if (pieceData.length !== piece.length) {
      console.error(`❌ Piece ${pieceIndex}: size mismatch (got ${pieceData.length}, expected ${piece.length})`);
    ...
    console.log(`✅ Piece ${pieceIndex} verified and completed (${piece.length} bytes)`);
    return true;
  }
   ...
   */
  getMissingBlocks(pieceIndex: number, blockSize: number = 16384): Array<{ begin: number; length: number }> {
    const piece = this.pieces.get(pieceIndex);
    if (!piece || this.completedPieces.has(pieceIndex)) {
      return [];
    }
  ...
  getStats() {
    ...
    const totalRequested = Array.from(this.pieces.values())
      .reduce((sum, piece) => sum + piece.requested.size, 0);

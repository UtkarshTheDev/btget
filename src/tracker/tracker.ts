...
async function getPeersHttp(
	...
	try {
		...
		if (Buffer.isBuffer(decoded.peers) || decoded.peers instanceof Uint8Array) {
			const peerBuffer = Buffer.isBuffer(decoded.peers)
				? decoded.peers
				: Buffer.from(decoded.peers);
		...
		if (decoded.peers6 && Buffer.isBuffer(decoded.peers6)) {
			try {
				const peers6 = group(decoded.peers6, 18)
					.map((buf) => {
...
function udpSend(
	...
	socket.send(msg, 0, msg.length, port, url.hostname, (err: Error | null) => {
		if (err) {
			callback(err);
		} else {
			callback();

...
describe('CLI Commands', () => {
  ...
  describe('info command', () => {
    ...
    it('should show torrent info for valid file', async () => {
      ...
      for (const torrent of testTorrents) {
        try {
          const { stdout } = await execAsync(`node ${CLI_PATH} info "${torrent}"`);
  ...
  describe('download command', () => {
    it('should validate torrent file exists before download', async () => {
      try {
        await execAsync(`node ${CLI_PATH} download nonexistent.torrent -o /tmp/test`);
        throw new Error('Should have failed');
      } catch (error: any) {
        expect(error.stderr || error.stdout).toContain('Error');
      }
    ...
    it('should accept output directory parameter', async () => {
      ...
      try {
        const { stdout, stderr } = await execAsync(`timeout 2 node ${CLI_PATH} download --help`);
    ...
    it('should support wget-style direct download', async () => {
      ...
      for (const torrent of testTorrents) {
        try {
          ...
          const { stdout, stderr } = await execAsync(`timeout 3 node ${CLI_PATH} "${torrent}" -o /tmp/test_direct 2>&1 || true`);
          const output = stdout + stderr;
          
          if (output.includes('Processing torrent file:') || 
